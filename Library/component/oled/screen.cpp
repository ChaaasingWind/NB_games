...
void screen::screen_run()
{
    ...
    _ctx.playing_mode             = ctx._playing_mode;
    _ctx.playing_state            = ctx.current_playing_state;
    _ctx.music_index              = ctx.current_music_index;
    _ctx.index                    = ctx.current_index;
    ...
    //状态机运行
    if(_ctx.state == menu::MenuState::MAIN)
    {
        screenFsm.change_state(&_mainScreen);
    }
    else if(_ctx.state == menu::MenuState::MUSIC)
    {
        screenFsm.change_state(&_musicScreen);
    }
    else if(_ctx.state == menu::MenuState::STANDBY)
    {
        screenFsm.change_state(&_standbyScreen);
    }
    else if(_ctx.state == menu::MenuState::PLAYING_MUSIC)
    {
        screenFsm.change_state(&_playingMusicScreen);
    }
    else if(_ctx.state == menu::MenuState::SETTINGS)
    {
        screenFsm.change_state(&_settingsScreen);
    }
    else if(_ctx.state == menu::MenuState::_DEBUG)
    {
